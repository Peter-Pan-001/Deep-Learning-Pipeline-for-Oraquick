...
insti_segmentation_model = tf.estimator.Estimator(
...
insti_segmentation_model.train(
        input_fn=train_input_fn,
        #steps=TRAINING_STEPS, # uncomment to train with number of training steps instead of number of epochs
        hooks=[logging_hook])

# 8) Export the model
insti_segmentation_model.export_savedmodel(EXPORT_DIR, unet.json_serving_input_receiver_fn)
...
eval_results = insti_segmentation_model.evaluate(input_fn=eval_input_fn)
...
test_results = insti_segmentation_model.predict(input_fn=eval_input_fn, yield_single_examples=False)
